...
class ACFileValidator
{
    public static function validate(string $fileName, string $rowData, int $rowNumber, string $batchId): void
    {
        ...
        $cols = [
            ...
            9 => 'Columna 10: codigo de diagnostico principal',
            10 => 'Columna 11: Código del diagnóstico relacionado No. 1',
            11 => 'Columna 12: Código del diagnóstico relacionado No. 2',
            12 => 'Columna 13: Código del diagnóstico relacionado No. 3',
        ...
        if ($numFactura === '') {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_001', $cols[0], '');
        }
        if ($codPrestador === '') {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_002', $cols[1], '');
        }

        $allowedTypes = ['CC', 'CE', 'CD', 'PA', 'SC', 'PE', 'RE', 'RC', 'TI', 'CN', 'AS', 'MS', 'DE', 'PT', 'SI'];
        if (! in_array($tipoId, $allowedTypes)) {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_003', $cols[2], $tipoId);
        }

        if ($numIdUsuario === '') {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_003_ID', $cols[3], '');
        } // New ID needed in ErrorCodes
        ...
        if (! in_array($finalidad, $allowedFinalidad)) {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_006', $cols[7], $finalidad);
        }
        ...
        if ($diagPpal === '') {
            self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AC_ERROR_009', $cols[9], '');
        }
