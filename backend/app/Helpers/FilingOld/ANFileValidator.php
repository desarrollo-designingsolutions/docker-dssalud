...
class ANFileValidator
{
    public static function validate(string $fileName, string $rowData, int $rowNumber, string $batchId): void
    {
        ...
        if ($numFactura === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_001', $cols[0], '');
        if ($codPrestador === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_002', $cols[1], '');
        ...
        if ($fecNac === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_004', $cols[4], '');
        if ($horaNac === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_005', $cols[5], '');
        if ($edadGest === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_006', $cols[6], '');
        ...
        if ($peso === '') self::logError($batchId, $rowNumber, $fileName, $data, 'FILE_AN_ERROR_011', $cols[9], '');
    }
